...
def halaman_awal():
    cursor = db.cursor()
    cursor.execute('select * from tbl_students_0569')
...
def proses_tambah():
    ...
    cur = db.cursor()
    cur.execute('INSERT INTO tbl_students_0569 (nim, nama, jk, jurusan, alamat) VALUES ( %s, %s, %s, %s, %s)', (nim, nama, jk, jurusan, alamat))
    db.commit()
...
def proses_ubah():
    ...
    alamat = request.form['alamat']
    cur = db.cursor()
    sql = "UPDATE tbl_students_0569 SET nim=%s, nama=%s, jk=%s, jurusan=%s, alamat=%s WHERE id=%s"
...
def hapus_data(id):
    cur = db.cursor()
    cur.execute('DELETE from tbl_students_0569 where id=%s', (id,))
    db.commit()
    return redirect(url_for('halaman_awal'))
